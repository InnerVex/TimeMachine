...
Select::Select(QDateTime sDateTime)
{
    //select(sDateTime);
...
qint32 Select::selectFileId(QDateTime sDateTime)
{
    auto_ptr<database> db (create_database ());
    QString e;
...
qint32 Select::selectOffset(QDateTime sDateTime)
{
    auto_ptr<database> db (create_database ());
...
qint32 Select::selectPathId(QDateTime sDateTime)
{
    auto_ptr<database> db (create_database ());
...
qint32 Select::selectSourceId(QDateTime sDateTime)
{
    auto_ptr<database> db (create_database ());
...
QString Select::selectFile(QDateTime sDateTime)
{
    auto_ptr<database> db (create_database ());
    qint32 sFileId;
    QString sFileName = "",e;
    ...
    {
        {
            ...
            for (result::iterator i (r.begin()); i != r.end(); ++i)
            {
                ...
                if(sFileName == "")
                {
                    e = "File for time = " +sDateTime.toString() +" does't found";
                    throw e;
                }
...
QString Select::selectPath(QDateTime sDateTime)
{
    auto_ptr<database> db (create_database ());
    qint32 sPathId;
    QString sPath;
...
QString Select::selectSourceName(QDateTime sDateTime){
    auto_ptr<database> db (create_database ());
    qint32 sSourceId;
    QString sSourceName;
    try
...
QString Select::selectSourceAdress(QDateTime sDateTime){
    auto_ptr<database> db (create_database ());
    qint32 sSourceId;
